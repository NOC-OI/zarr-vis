...
export default function CircleRenderer({
  ...
  function addDragHandleListeners() {
    const onMouseMove = e => {
      let r = distance(map.unproject(e.point).toArray(), [center.lng, center.lat], { units });
  ...
  function setCircle() {
    ...
    const handleXY = (() => {
      ...
      let coordinates = rhumbDestination([center.lng, center.lat], radius, guidelineAngle).geometry
        .coordinates;
    ...
    svgRadiusText
      ...
      .attr('transform', `rotate(${-1 * guidelineAngle + 90}) ` + `translate(0, ${translateY})`);
