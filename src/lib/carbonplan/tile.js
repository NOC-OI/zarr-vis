import { getBandInformation, getChunks, getSelectorHash, keyToTile } from './utils';

class Tile {
  constructor({ key, loader, shape, chunks, dimensions, coordinates, bands, initializeBuffer }) {
    this.key = key;
  ...
  populateBuffersSync(selector) {
    ...
    this.bands.forEach(band => {
      ...
      if (chunks.length !== 1) {
        throw new Error(
          `Expected 1 chunk for band '${band}', found ${chunks.length}: ${chunks.join(', ')}`
        );
      }
        ...
        const indices = this.dimensions
          ...
          .map((d, i) => {
            if (info[d] === undefined) {
              ...
              const value = info[d];
              return (
                this.coordinates[d].findIndex(coordinate => coordinate === value) % this.chunks[i]
              );
            }
          });
        ...
        (accum, count, i) => {
          ...
          if (['x', 'lon'].includes(dimension)) {
            ...
            const selectorIndices = selectorValues
              ...
              .filter(index => chunkOffset <= index && index < chunkOffset + count);
