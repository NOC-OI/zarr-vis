...
export const createTiles = (regl, opts) => {
  ...
  function Tiles({
          ...
          keysToRender.forEach(keyToRender => {
            ...
            offsets.forEach(offset => {
              ...
              const alreadySeenOffset = accum[keyToRender].find(
                prev => prev[0] === adjustedOffset[0] && prev[1] === adjustedOffset[1]
              );
    ...
    this.queryRegion = async (region, selector) => {
      ...
      const lon = [];
      const resultDim =
        this.ndim - Object.keys(selector).filter(k => !Array.isArray(selector[k])).length;
      if (resultDim > 2) {
        results = {};
      } else {
        results = [];
      ...
      tiles.map(key => {
        ...
        for (let i = 0; i < this.size; i++) {
          for (let j = 0; j < this.size; j++) {
            ...
            const pointCoords = [
              mercLon,
              this.projection === 'equirectangular' ? lat0 - this.order[1] * stepDeg * j : mercLat
            ];
